...
提取方式（按优先级）：
  D9：双轨制
    → 轨道A：从D1度数用公式计算（100%准确）
       D9星座 = floor(绝对经度 / 3.333) % 12 + 1
    → 轨道B：从PDF/截图视觉识别
    → 两轨交叉验证：一致=✅，不一致=以公式为准并标注
  D10/D4/D5：
    → 优先从文本层提取（如JH表格有分盘数据）
    → 文本层无数据 → 视觉提取（见下方规则）
...
视觉提取铁规（D10/D4/D5）：
  ⚠️ 禁止自动crop局部区域！AI估算坐标经常偏移导致读错。
  正确做法：
    1. 用PyMuPDF渲染包含分盘图的PDF页面为高清PNG（300DPI）
       page.get_pixmap(dpi=300) → 保存为 pageN_DXX.png
    2. 将完整页面图片交给AI视觉识别
       AI从整页中自行定位目标分盘图表，不需要预裁剪
    3. 如果一页有多个分盘，用文字提示AI看哪个：
       "请从这页中读取D10(Dasamsha)盘的行星位置"

  校验12(Ra-Ke)不通过时的兜底：
    → 先用视觉二次确认（重新看整页图）
    → 仍然不通过 → 请用户截屏该分盘图发来：
      "D10的Ra-Ke位置校验未通过，请帮我截一张D10盘的高清图发来，
       我重新读取一次。"
    → 用户截屏通常准确率显著高于PDF自动渲染
