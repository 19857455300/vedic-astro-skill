...
必需数据：
  ...
  4. 时间精度（用户声明的±X分钟）
  5. Lagna度数和星座
  6. Vimsottari Dasha完整时间线：calc路径必须验证MD=9/AD=81/PD=729且三层`[start,end)`连续；
...
`--event-date`是可选、可重复参数，格式`YYYY-MM-DD`或`YYYY-MM`。它只输出段首/段末的
活跃MD/AD/PD差异审计，不改扫描区间、不产生分钟结论。只记得年的事件不传此参数。
...
⚠️ **读扫描结果第一步：核两点法 Dasha 在不在**——存盘文件(rectification_scan.md)末尾应有「## 两点法 Dasha（段首/段末）」两条完整时间线。若看到 **`DASHA_UNAVAILABLE`** 或「Dasha 计算失败」→ **停**：硬腿数据源缺失，❌ 禁在无两点法 Dasha 下进 3c 段内定位/定盘（只剩结构表 = 退化成纯软腿匹配 = 过拟合）。先排查 python/swisseph/jhora 依赖、重跑 time_scan 拿到两点 Dasha 再继续。
