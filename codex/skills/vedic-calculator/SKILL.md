...
- Python 3.10 ~ 3.13（推荐 Python 3.12；pysweph 为 C 扩展，**3.14 暂不支持**）
...
运行前检查依赖是否可用（按优先级）：
  ...
  1. 检查 <skill目录>/venv/ 是否存在并可用：
     - macOS/Linux: <skill目录>/venv/bin/python
     - Windows: <skill目录>\venv\Scripts\python.exe
     找到 → 尝试 import swisseph → 成功 → 直接使用

  2. 如果 skill 目录 venv 不可用，自动创建 venv（运行 setup_env.py）：
...
> ⚠️ 排盘命令不要使用系统 `python3`，优先且默认使用 skill 目录内的 venv。系统解释器只用于 venv 不存在时启动 `setup_env.py`。
> ⚠️ 如果系统默认 Python 是 3.14，setup_env.py 会自动查找 3.10~3.13，推荐使用 3.12。
...
优先调用正式 CLI，不要临时创建 `generate_structured_data.py`：

```bash
# macOS/Linux（优先）
<skill目录>/venv/bin/python <skill目录>/scripts/generate_structured_data.py \
  --date YYYY-MM-DD \
  --time HH:MM \
  --place "城市名" \
  --lat LAT \
  --lon LON \
  --tz TIMEZONE \
  --output structured_data.md \
  --gender "男/女" \
  --relationship "单身/恋爱中/已婚" \
  --time-precision "精确到分钟" \
  --time-source "用户提供"

# Windows
<skill目录>\venv\Scripts\python.exe <skill目录>\scripts\generate_structured_data.py ...
```

> ⚠️ **禁止自己手写或拼接 structured_data.md！** `formatter.py` 是唯一格式化来源，CLI 内部必须调用 `format_structured_data()`。
> ⚠️ 不要使用系统 `python3` 执行排盘 CLI。若 venv 缺失，先运行 `setup_env.py` 创建 skill 专用 venv。
...
命令必须以退出码 0 结束，并输出 `SAV total: 337`、Lagna、Moon sign 和 `structured_data.md generated successfully`。
