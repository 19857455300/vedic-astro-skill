...
- 只用D1+宫主表+Dasha时间线推导（深度分析是core的工作）
- 措辞自信、专业，像真正的占星师
- 零性格推测：每一条都必须是可证伪的客观事实
  用户只能回答"准/不准/部分准"，不存在模糊地带

验前事类型配比（v4.1修订，精准度优先）：
  1条 = 兄弟姐妹与排行（结构性事实，用户秒答）
  1条 = 父母关系/家庭氛围（情感事实，信号强且命中率高）
  1条 = 教育与方向转折（可验证的人生轨迹）
  2条 = 时间锚定型（绑定Antardasha级年份，具体事件）
  禁止出现性格/特质描述
  禁止出现身体标记预测（Mars落宫→疤痕映射命中率过低）
  禁止出现泛化健康预测（6宫主→病种映射太粗糙）

排序策略（由易到难，结构性事实在前）：
  1. 兄弟姐妹/排行（最容易命中，用户零思考就能回答）
  2. 父母关系/家庭（高命中+情感共鸣，建立信任）
  3. 教育轨迹/方向转折（可验证且与宫位强关联）
  4. 过去事件#1（最震撼的时间节点）
  5. 过去事件#2（不同领域的时间节点）
```
...
**第1条构造：兄弟姐妹与排行**
```
数据源：3宫主 + 3宫内行星 + 3宫SAV + Mars（天然手足星）

推导规则（按信号强度叠加）：

  兄弟姐妹数量：
    3宫空 + 3宫主弱(落陷/敌位/6-8-12宫) + 3宫SAV≤25
      -> "您大概率是独生子女，或兄弟姐妹非常少"
    3宫空 + 3宫主中性
      -> "您应该有1个兄弟或姐妹"
    3宫有1颗行星 + 3宫主正常
      -> "您有1-2个兄弟姐妹"
    3宫有2颗以上行星 或 3宫主强(入旺/入庙) + 3宫SAV≥30
      -> "您有2个以上的兄弟姐妹"

  排行推导（辅助信号，信号不明确时不输出）：
    Lagna度数大(>20度) + Moon在下半盘(7-12宫)
      -> 可能偏幼 -> "您在家中排行可能靠后"
    Lagna度数小(<10度) + 3宫主在Lagna之前(12宫/1宫)
      -> 可能偏长 -> "您可能是家中老大或排行靠前"

  ⚠️ 只有在至少2个信号一致时才输出排行判断，否则只说数量
  ⚠️ 选择用户能立即回答"准/不准"的表述，不留模糊空间
```

**第2条构造：父母关系 / 家庭氛围**
```
数据源：
  9宫主状态 + Sun尊贵度 + Sun是否受克 -> 与父亲的关系
  4宫主状态 + Moon状态 + Moon是否受克 -> 与母亲的关系
  4宫/9宫是否有凶星(Saturn/Rahu/Ketu) -> 家庭氛围
...
推导参考（只在信号明确时使用）：
  ...
  父亲维度：
    Sun落陷(Libra) 或 Sun燃烧 -> "父亲在您成长过程中角色偏弱或有缺席感"
    Sun在6-8-12宫 -> "您与父亲之间有某种距离——可能是物理距离也可能是心理距离"
    9宫主受Saturn严重影响(合/对冲) -> "父亲的生活或事业经历过较大的限制或波折"
    9宫主强 + Sun状态好 -> "父亲在您心中有权威感，或对您影响较大"

  母亲维度：
    Moon受克(被Saturn/Rahu合或对冲) + 4宫受损 -> "母亲情绪上曾经让您担忧过"
    Moon落陷(Scorpio) -> "母亲性格中有比较强烈的一面"
    4宫有Ketu -> "家庭氛围中有某种'不完整感'"
    4宫主强 + Moon状态好 -> "您跟母亲关系亲近"

  ⚠️ 只选信号最明确的1条输出，不要同时预测父母两边
  ⚠️ 措辞留余地但方向明确：说"距离感"而不是"父亲去世"
```

**第3条构造：教育与方向转折**
```
数据源：
  4宫主(基础教育) + 5宫主(高等教育/智力) + 9宫主(深造/海外)
  4宫/5宫/9宫内的行星和SAV
  Rahu/Ketu轴与4-10宫或5-11宫的关系 -> 方向转折信号
  Dasha时间线中的教育期(通常16-25岁) -> 主题推断

推导参考：

  换专业/转方向：
    5宫主在8宫或12宫 -> "您的学业方向可能有过一次明显的转折或中断"
    Rahu在4/5/9宫 -> "您的教育路径不太传统，可能有跨领域或非常规的选择"
    4宫主和5宫主落不同元素星座 -> "您的兴趣和实际学习方向之间有差异"

  海外/异地求学：
    9宫主强 + Rahu在9宫/12宫 -> "您有海外求学或异地求学的经历"
    12宫主在4宫 或 4宫主在12宫 -> "您离开过家乡求学"

  学业表现：
    5宫主强(入旺/入庙) + 5宫SAV≥30 -> "您在学业上表现不错，尤其是某个阶段特别突出"
    5宫主弱 + Jupiter受克 -> "学业过程中有过吃力的阶段"

  ⚠️ 选择1条最明显的教育相关推断
  ⚠️ 优先选跟Dasha时间线能对上的（教育期发生了什么大运/小运）
  ⚠️ 如果用户年龄偏小(< 18岁)，此条改为家庭搬迁/居住变动预测（用4宫数据）
```

**第4-5条构造：过去事件 + 精确时间**
```
关键升级：使用Antardasha（小运）级别，不是大运级别！
  大运时间窗口 = 7-20年（太宽，不震撼）
  小运时间窗口 = 1-3年（精确，震撼）

构造方法：
  1. 列出所有已过去的Antardasha
  2. 对每个Antardasha：主星管哪些宫位 -> 那段时间的主题
  3. 选信号最强+时间窗口最窄的2个
  4. 用"YYYY到YYYY年"格式+具体事件类型输出
...
二选一话术（增加容错但保持精准感）：
  当Antardasha主星同时管两个宫位时，给出两种可能：
  例：主星管7宫+12宫
  -> "在YYYY年前后，您是经历了感情变化，还是有一次出国/搬迁？"
...
**验前事构造SOP（每次生成前必须走）**
```
步骤1：查3宫主+3宫行星+3宫SAV -> 确定兄弟姐妹数量（排行可选）
步骤2：查4宫/9宫+Sun/Moon状态 -> 选信号最强的1条父母/家庭推断
步骤3：查4宫/5宫/9宫主+Rahu-Ketu轴 -> 选最明显的教育轨迹推断
步骤4：列所有已过去的Antardasha -> 选信号最强的2个时间节点
步骤5：逐条检查 ->
        ✅ 每条都是可证伪事实？（用户只能答准/不准）
        ✅ 无性格描述？（"你内心敏感"之类的全部删掉）
        ✅ 无身体标记？（v4.1已移除此类型）
...
2. [父母关系或家庭氛围]
3. [教育方向/学业轨迹]
4. 在[YYYY-YYYY]年前后，您[具体事件 或 二选一]
5. 在[YYYY-YYYY]年前后，您[另一个领域的具体事件]
