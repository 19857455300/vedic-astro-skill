...
**评分与后续决策**：
```
R1反馈后，将偏差记入修正日志，然后根据命中率决定：

  全部命中(5/5, 4/4) → 直接进入分析
  大部分命中(4/5)    → 建议做一轮方向验证（R2），可跳过
  有偏差(≤3/5)       → 推荐做方向验证（R2），或建议时间校准
...
> R2不是"重新猜"，也不是从弱信号池捞新的。它是：
> 1. 基于R1的❌条目，对同一个信号换一个解读方向
> 2. 目标：确认信号本身是否存在，只是表达方式不同
> 3. R2精度可能比R1更高（因为有用户反馈做方向指引）
...
步骤1: 反馈分析（展示给用户看）
  逐条分析R1结果：
  - ✅条目：确认信号方向正确，一句话即可
  - ❌条目：分析信号为什么偏了——是方向错了还是信号本身不存在？
    （注意：不是辩解，是客观分析偏差原因）
  - ❓/部分准：分析哪部分准、偏差在哪里
  最后用1-2句话总结：调整了什么分析方向

步骤2: 对❌条目换角度出推断
  每条❌生成1条方向验证推断（不超过3条总计）
  示例：
    R1: "父亲严肃"(9宫Saturn) ❌
    R2: "9宫Saturn的压力，是否体现在父亲在经济/健康方面给家庭带来过负担？"
    → 同一个9宫Saturn信号，不同表达方向
...
**R2 方向验证信号选择规则**：
```
R2的目的不是"再猜新的"，而是"对R1的❌条目换角度验证同一信号"。

1. 只针对R1的❌/❓条目，每条换一个解读方向
   → R1: "父亲严肃"(9宫Saturn) ❌
   → R2: "9宫Saturn的压力，是否体现在父亲在经济/健康方面给家庭带来过负担？"
   → 同一个信号，不同表达方向
...
4. R2最多出3条（仅针对R1的❌/❓，不超过❌条目数）

5. R2的❌直接记入修正日志，不再追加R3
```

**综合评分（R1+R2）**：
```
合并评分 = (R1命中 + R2命中) / (R1总数 + R2总数)

R2结束后（或用户跳过R2后），根据合并评分决定：
  综合 ≥ 70% → 时间可信度=高，进入core
  综合 50-70% → 时间可信度=中，进入core但标注校准率
  综合 < 50%  → 建议时间校准
    → 用户接受 → 转vedic-rectifier → 校准后触发【校准后重验】
    → 用户拒绝 → 进入core，标注"时间可信度=低"
```

**⚠️ 验前事安全约束**：
```
1. 最多两轮（R1 + R2方向验证），不追加R3
2. 不能用用户反馈的信息"装作"独立推断
   → 用户说了"我2024分手" → 不能再说"您2024年感情有变"当命中
3. ❌不能再解释 → 不准就是不准，记入修正日志
4. R1和R2都要记修正日志：AI预测→信号调整
5. R2反馈处理同样遵守上方的"反馈处理硬约束"
6. ⚠️ 反锚定规则：R2推断必须从盘面推导，
   不能因为"用户说过X"就推出Y。
```
...
→ 有❌条目 → 自动执行一轮R2方向验证（同Step 5b规则）
→ 全命中或R2完成后 → 综合评分 → 进入core
→ 修正日志持续累积
```
