...
文件:   structured_data.md（分3次写入，禁止合并）
```

**⚠️⚠️⚠️ 分次写入铁规（禁止违反）**：
```
structured_data.md 必须分3次写入，每次≤200行。
⚠️ 禁止将多次写入合并为一次！合并会导致output超限崩溃。
⚠️ 每次写入后必须在聊天框输出进度消息，再继续下一步。

第1次写入（Path B门控完成后）：
  write_to_file → 元信息 + 用户信息 + D1基础数据 + 量化数据 + 分盘数据 + 校验结果
  写入后立即输出: "✅ 基础数据已写入structured_data.md。开始预分析..."
  → 然后才执行Step 4

第2次写入（Step 4预分析完成后）：
  追加写入 → 预分析（尊贵度 + 相位 + 宫主表 + 信号预扫）
  写入后立即输出: "✅ 预分析已追加写入。开始生成验前事..."
  → 然后才执行Step 5
...
⚠️ 预分析完成后执行【第2次写入】（禁止与第1次合并！）：
追加写入structured_data.md → 预分析section（尊贵度表 + 相位表 + 宫主表 + 信号预扫）。写入后在聊天框输出: "✅ 预分析已追加写入。开始生成验前事..."
